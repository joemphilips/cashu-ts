...
var Bt = (e) => {
	let t;
	return t = /^[a-fA-F0-9]+$/.test(e) ? ei(e) % BigInt(2 ** 31 - 1) : D.toBigInt(be(e)) % BigInt(2 ** 31 - 1), t;
}, Vt = "m/0'/0'/0'";
function Ht(t) {
	let n = {};
...
function Wt(e, t, n) {
	...
	for (t && (d = g.fromMasterSeed(t)); c < e;) {
		let e = (2n ** c).toString();
		if (d) {
			let t = s === 2 ? `${Vt}/${c}'` : `${Vt}/${c}`, n = d.derive(t).privateKey;
			if (n) u[e] = s === 2 ? a(n) : n;
...
function qt(e, t) {
	let n = Array.isArray(e) ? e : [e], r = I(t), i = r[0];
...
function nn(e, t) {
	...
	return {
		blinded: e.map((e, t) => {
			let r = P(e), i = on(r, n, t), a = r.add(l.Point.BASE.multiply(i));
...
function on(e, t, n) {
	let r = e.multiply(t).toBytes(!0).slice(1), i = new Uint8Array([n & 255]), o = D.toBigInt(a(D.concat(tn, r, i)));
	if ((o === 0n || o >= l.Point.CURVE().n) && (o = D.toBigInt(a(D.concat(tn, r, i, new Uint8Array([255])))), o === 0n || o >= l.Point.CURVE().n)) throw new v("P2BK: tweak derivation failed");
	return o;
}
//#endregion
...
function vn(t, r, i) {
	...
	if (hn(t.witness).some((e) => nt(e, i, s))) throw new v(`Proof already signed by [02|03]${s}`);
	let u = tt(i, r), d = gn(t.witness), f = {
...
function yn(e, t, n) {
	...
	return n = n ?? t.secret, hn(t.witness).some((t) => nt(t, n, e));
}
function bn(e, t = C, n) {
	...
	let o = hn(e.witness), s = Mn(r), c = Nn(s), l = Pn(r, c, a), u = Fn(r, c, a), d = it(o, n, i), f = a.length ? it(o, n, a) : [], p = {
		locktime: s,
		lockState: c,
		main: {
			pubkeys: i,
...
var Bn = (e, t, n, r) => {
	...
	let n = l.Point.Fn.fromBytes(t), i = l.Point.BASE.multiply(n), a = e.multiply(n), o = zn(t, i, e, a), s = Dt([
...
function Xn(e, t, n) {
	let r = `${Un}/${Bt(t)}'/${n}'`, i = e.derive(r), a = i.deriveChild(0).privateKey, o = i.deriveChild(1).privateKey;
	/* c8 ignore next */
...
function Qn(e, t, n) {
	...
	let i = _(a, e, D.concat(t, D.fromHex("01"))), o = D.toBigInt(i), s = o >= Wn ? o - Wn : o;
	/* c8 ignore next */
...
function Rr(e) {
	let t = [];
	if (e.id && t.push(V(ur, H(e.id))), e.amount !== void 0 && t.push(V(dr, zr(e.amount))), e.unit && (e.unit === "sat" ? t.push(V(fr, new Uint8Array([0]))) : t.push(V(fr, H(e.unit)))), e.singleUse !== void 0 && t.push(V(pr, Br(+!!e.singleUse))), e.mints && e.mints.length > 0) for (let n of e.mints) t.push(V(mr, H(n)));
	if (e.description && t.push(V(hr, H(e.description))), e.transports && e.transports.length > 0) for (let n of e.transports) t.push(V(gr, Ur(n)));
	if (e.nut10 && e.nut10.length > 0) for (let n of e.nut10) t.push(V(_r, Wr(n)));
...
function Ur(e) {
	...
	t.push(V(yr, r));
	let a = [...i, ...e.tags || []];
	if (a.length > 0) for (let e of a) t.push(V(br, Gr(e)));
...
function Wr(e) {
	let t = [], n = Hr(e.kind);
	if (t.push(V(Cr, Br(n))), t.push(V(wr, H(e.data))), e.tags && e.tags.length > 0) for (let n of e.tags) t.push(V(Tr, Gr(n)));
...
function fi(e, t) {
	...
	switch (o) {
		case 0: {
			let t = a(pi(...Object.entries(e).sort(([e], [t]) => k.from(e).compareTo(t)).map(([, e]) => n(e))));
...
function Si(e, t, r) {
	...
	try {
		...
		return Vn(new TextEncoder().encode(e.secret), t, P(e.C), P(a));
	} catch {
		return !1;
	}
}
...
function wi(e) {
	let t = new TextEncoder(), n = J(e.proofs), r = Ae(si({
		...e,
		proofs: n
	}));
...
async function Zi(e) {
	let { endpoint: t, requestBody: n, headers: r, requestTimeout: i, responseBodyBytesLimit: a, onResponseMeta: o, cached_endpoints: s, ttl: c, logger: l, ...u } = e, d = n ? A.stringify(n) : void 0, f = Ii(d, r), p = e.signal ?? void 0;
...
var fa = class {
	...
	normalizeMintQuoteBolt12Fields(e) {
		hi(e, "amount"), e.amount = e.amount === null ? null : k.from(e.amount), e.expiry = Y(e.expiry, "mintQuoteBolt12.expiry", null), e.amount_paid = k.from(e.amount_paid), e.amount_issued = k.from(e.amount_issued);
	}
...
var Q = class e {
	...
	static createSingleP2PKData(t, n, r) {
		let i = k.from(n), a = fn(t), o = Array.isArray(a.pubkey) ? a.pubkey : [a.pubkey], s = a.refundKeys ?? [], c = a.requiredSignatures ?? 1, l = a.requiredRefundSignatures ?? 1, u = a.hashlock, f = typeof u == "string" && u.length > 0, p = f ? u : o[0], h = f ? o : o.slice(1), g = s, _;
		if (t.blindKeys) {
		...
		let ee = new TextEncoder().encode(S), { r: w, B_: te } = wa(ee, r);
		return new e(new va(i, te, r).getSerializedBlindedMessage(), w, ee, _);
	}
	...
	static createSingleRandomData(t, n) {
		let r = k.from(t), i = d(m(32)), a = new TextEncoder().encode(i), { r: o, B_: s } = wa(a, n);
		return new e(new va(r, s, n).getSerializedBlindedMessage(), o, a);
	}
	static createDeterministicData(e, t, n, r, i) {
		let a = U(e, r.keys, i), o = Jn(t, r.id);
		return a.map((e, t) => Ca(e, r.id, o(n + t)));
	}
	static createSingleDeterministicData(e, t, n, r) {
		return Ca(e, r, qn(t, r, n));
	}
...
function Ca(e, t, n) {
	let r = k.from(e), i = d(n.secret), a = new TextEncoder().encode(i), { r: o, B_: s } = wa(a, t, D.toBigInt(n.blindingFactor));
	return new Q(new va(r, s, t).getSerializedBlindedMessage(), o, a);
}
function wa(e, t, n) {
	if (F(t)) {
		let t = yt(e, n);
...
var Ea = class e {
	...
	lockUntil(e) {
		return this.locktime = Ta(e), this;
	}
...
function Ia(e) {
	return e instanceof Error ? e : new v(typeof e == "string" ? e : Fa(e), { cause: e });
}
function La() {
...
var Ra = class {
	...
	waitUntilPaid(e, t, n, r = "Timeout waiting for paid") {
		return new Promise((i, a) => {
			let o = null, s = null, c = !1, l = (e) => {
				c || (c = !0, $(o), s && (clearTimeout(s), s = null), n?.signal && n.signal.removeEventListener("abort", u), e && a(Ia(e)));
			}, u = () => l(La());
			if (n?.signal) {
		...
		return new Promise((n, r) => {
			let i = Array.from(new Set(e)), a = /* @__PURE__ */ new Map(), o = null, s = null, c = !1, l = !1, u = (e) => {
				if (!l) {
					...
					a.clear(), o && (clearTimeout(o), o = null), t?.signal && t.signal.removeEventListener("abort", d), e && r(Ia(e));
				}
			}, d = () => u(La());
			if (t?.signal) {
	...
	send(e, t) {
		return new Ba(this.wallet, e, t);
	}
	receive(e) {
		return new Va(this.wallet, e);
	}
	mintBolt11(e, t) {
		return new Ha(this.wallet, "bolt11", e, t);
	}
	mintBolt12(e, t) {
		return new Ha(this.wallet, "bolt12", e, t);
	}
	mintOnchain(e, t) {
		return new Ha(this.wallet, "onchain", e, t);
	}
	meltBolt11(e, t) {
		return new Ua(this.wallet, "bolt11", e, t);
	}
	meltBolt12(e, t) {
		return new Ua(this.wallet, "bolt12", e, t);
	}
	meltOnchain(e, t) {
		return new Wa(this.wallet, e, t);
	}
}, Ba = class {
	constructor(e, t, n) {
...
var Ka = "__PENDING__", qa = class e {
	constructor(e, t) {
		this._seed = void 0, this._unit = "sat", this._mintInfo = void 0, this._denominationTarget = 3, this._secretsPolicy = "auto", this._boundKeysetId = Ka, this._requireSigDleq = !1, this.ops = new za(this), this.on = new Ra(this), this._logger = t?.logger ?? C, this._selectProofs = t?.selectProofs ?? Da, this._outputDataCreator = t?.outputDataCreator ?? new Oa(), this.mint = typeof e == "string" ? new ha(e, {
			authProvider: t?.authProvider,
			logger: this._logger
		}) : e, this._unit = t?.unit ?? this._unit, this._boundKeysetId = t?.keysetId ?? this._boundKeysetId, t?.bip39seed && (this.failIf(!(t.bip39seed instanceof Uint8Array), "bip39seed must be a valid Uint8Array", { bip39seed: t.bip39seed }), this._seed = t.bip39seed), this._secretsPolicy = t?.secretsPolicy ?? this._secretsPolicy, t?.counterSource ? this._counterSource = t.counterSource : this._counterSource = new Ma(t?.counterInit), this.counters = new Pa(this._counterSource), this._keyChain = new _a(this.mint, this._unit), this._denominationTarget = t?.denominationTarget ?? this._denominationTarget, this._requireSigDleq = t?.requireSigDleq ?? this._requireSigDleq, this.ctf = t?.enableCtf ? this.createCtfFacade() : void 0;
	}
	createCtfFacade() {
		return {
	...
	finishInit() {
		if (this._logger.debug("KeyChain", { keychain: this._keyChain.cache }), this._boundKeysetId === Ka) try {
	...
	get keysetId() {
		return this.failIf(this._boundKeysetId === Ka, "Wallet has no bound keyset. The mint may have no active keysets, or wallet was not initialized via loadMint or loadMintFromCache"), this._boundKeysetId;
	}
	getKeyset(e) {
	...
	configureOutputs(e, t, n, r = !1, i = []) {
		...
		if (s.length === 0 && o.length > 0 && (s = Aa(o, a, t.keys, this._denominationTarget)), s = U(a, t.keys, s), r) {
	...
	sendOffline(e, t, n) {
		...
		a && (i = i.filter((e) => F(e.id) || e.dleq != null)), this.failIf(q(i).lessThan(r), "Not enough funds available to send");
	...
	async checkProofsStates(e, t) {
		let n = new TextEncoder(), r = e.map((e) => F(e.id) ? M(n.encode(e.secret)).toHex(!0) : N(n.encode(e.secret)).toHex(!0)), i = [];
		for (let e = 0; e < r.length; e += 100) {
			let n = r.slice(e, e + 100), { states: a } = await this.mint.check({ Ys: n }, Ga(this.mint, t)), o = {};
			a.forEach((e) => {
				o[e.Y] = e;
			});
...
function eo(e) {
	...
	let n = A.stringify(t) ?? "{}";
	return `${Qa}${ve(D.fromString(n))}`;
}
function to(e, t) {
	if (!e.startsWith(Qa)) throw new v(`Invalid signing package: must start with "${Qa}"`);
	let n = e.slice(7), r;
	try {
	...
	let a = i, o = a.version;
	if (o !== Qa) throw new v(`Invalid signing package version: ${o}`);
	let s = a.type;
	...
	if (t?.validateDigest) {
		let e = $a(a.inputs, a.outputs, a.quote);
...
function ro(e) {
	...
	return ao("swap", e.inputs, t.map((e) => e.blindedMessage));
}
function io(e) {
	return ao("melt", e.inputs, e.outputData.map((e) => e.blindedMessage), e.quote.quote);
}
function ao(e, t, n, r) {
	let i = $a(t, n, r), a = $e(wn(t, n.map((e) => ({ blindedMessage: e })), r), !0);
...
var lo = {
	computeDigests: $a,
	extractSwapPackage: ro,
	extractMeltPackage: io,
	serializePackage: eo,
	deserializePackage: to,
	signPackage: no,
	signDigest: et,
	mergeSwapPackage: oo,
	mergeMeltPackage: so
...
//#endregion
export { k as Amount, O as AmountError, oe as AmountWithUnit, ae as AmountWithUnitError, Ya as AuthManager, lt as BLS_FR_ORDER, ut as BLS_G2_GENERATOR, ct as BLS_HASH_TO_CURVE_DST, v as CTSError, ha as CashuMint, ha as Mint, qa as CashuWallet, qa as Wallet, Ni as CheckStateEnum, ne as ConsoleLogger, y as HttpResponseError, A as JSONInt, _a as KeyChain, ga as Keyset, Ua as MeltBuilder, Wa as MeltOnchainBuilder, Mi as MeltQuoteState, Ha as MintBuilder, X as MintInfo, S as MintOperationError, ji as MintQuoteState, b as NetworkError, Ai as OIDCAuth, Q as OutputData, tn as P2BK_DST, Ea as P2PKBuilder, Yr as PaymentRequest, lr as PaymentRequestTransportType, x as RateLimitError, Va as ReceiveBuilder, Ba as SendBuilder, lo as SigAll, sn as SigFlags, ma as WSConnection, Pa as WalletCounters, Ra as WalletEvents, za as WalletOps, Lt as asBlsG1Point, It as asSecpPoint, qt as assertSecretKind, Cn as assertSigAllInputs, Tt as batchVerifyUnblindedSignatureBls, ri as bigIntStringify, Nt as blindMessage, yt as blindMessageBls, In as buildLegacyP2PKSigAllMessage, wn as buildP2PKSigAllMessage, $e as computeMessageDigest, Ft as constructUnblindedSignature, xt as constructUnblindedSignatureBls, Za as createAuthWallet, jt as createBlindSignature, St as createBlindSignatureBls, Hn as createDLEQProof, Na as createEphemeralCounterSource, tr as createHTLCHash, $n as createHTLCsecret, Wt as createNewMintKeys, un as createP2PKsecret, Mt as createRandomRawBlindedMessage, At as createRandomSecretKey, Kt as createSecret, Jn as createSecretAndBlindingFactorDeriver, yi as decodePaymentRequest, z as dedupeP2PKPubkeys, wt as deriveBatchWeights, Kn as deriveBlindingFactor, ot as deriveConditionalKeysetId, fi as deriveKeysetId, nn as deriveP2BKBlindedPubkeys, an as deriveP2BKSecretKey, rn as deriveP2BKSecretKeys, Gn as deriveSecret, qn as deriveSecretAndBlindingFactor, Ut as deserializeMintKeys, vi as deserializeProofs, rt as findSigningKey, Xt as getDataField, li as getDecodedToken, Ti as getDecodedTokenBinary, ai as getEncodedToken, ai as getEncodedTokenV4, wi as getEncodedTokenBinary, _t as getG2PubKeyFromPrivKey, ar as getHTLCWitnessPreimage, Xr as getKeysetAmounts, Bt as getKeysetIdInt, pn as getP2PKExpectedWitnessPubkeys, mn as getP2PKSigFlag, hn as getP2PKWitnessSignatures, kt as getPubKeyFromPrivKey, Yt as getSecretData, Jt as getSecretKind, $t as getTag, L as getTagInt, en as getTagScalar, Zt as getTags, ui as getTokenMetadata, it as getValidSigners, Qr as hasCorrespondingKey, yn as hasP2PKSignedProof, Qt as hasTag, Si as hasValidDleq, N as hashToCurve, M as hashToCurveBls, Dt as hash_e, ei as hexToNumber, ua as injectWebSocketImpl, Di as invoiceHasAmountInHRP, F as isBlsKeyset, ir as isHTLCSpendAuthorised, G as isObj, Tn as isP2PKSigAll, xn as isP2PKSpendAuthorised, W as isValidHex, K as joinUrls, Sn as maybeDeriveP2BKPrivateKeys, at as meetsSignerThreshold, fn as normalizeP2PKOptions, J as normalizeProofAmounts, gi as normalizeUrl, hi as nullIfUndefined, ti as numberToHexPadded64, er as parseHTLCSecret, R as parseP2PKSecret, I as parseSecret, gn as parseWitnessData, Ot as pointFromBytes, P as pointFromHex, Rt as pointFromHexAuto, ht as pointFromHexG1, gt as pointFromHexG2, zt as pointToHex, et as schnorrSignDigest, tt as schnorrSignMessage, nt as schnorrVerifyMessage, Da as selectProofsRGLI, Ht as serializeMintKeys, _i as serializeProofs, Bi as setGlobalRequestOptions, sr as signMintQuote, vn as signP2PKProof, _n as signP2PKProofs, mi as sortProofsById, U as splitAmount, bi as stripDleq, q as sumProofs, Pt as unblindSignature, bt as unblindSignatureBls, Bn as verifyDLEQProof, Vn as verifyDLEQProof_reblind, nr as verifyHTLCHash, rr as verifyHTLCSpendingConditions, cr as verifyMintQuoteSignature, bn as verifyP2PKSpendingConditions, Ci as verifyProofsForReceive, Gt as verifyUnblindedSignature, Ct as verifyUnblindedSignatureBls };
