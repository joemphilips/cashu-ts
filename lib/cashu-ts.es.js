...
function P(e) {
	return e.length !== 16 && e.length !== 66 || !G(e) ? !1 : e.startsWith("02");
}
...
var F = class e {
	...
	static createP2PKData(e, t, n, r) {
		return W(t, n.keys, r).map((t) => this.createSingleP2PKData(e, t, n.id));
	}
	...
	static createRandomData(e, t, n) {
		return W(e, t.keys, n).map((e) => this.createSingleRandomData(e, t.id));
	}
	...
	static createDeterministicData(e, t, n, r, i) {
		let a = W(e, r.keys, i), o = Zn(t, r.id);
...
function ir(e) {
	if (mr(e.receiveKeyset, "receive"), mr(e.offerKeyset, "offer"), e.receiveKeyset.id === e.offerKeyset.id) throw new v("CTF receive and offer keysets must differ");
	if (!Number.isSafeInteger(e.maxEntries) || e.maxEntries < 2) throw new v("maxEntries must be a safe integer of at least two");
	let t = [...fr(e.receiveKeyset, e.maxReceive, "receive"), ...fr(e.offerKeyset, e.maxChange, "change")];
	if (t.length > e.maxEntries) throw new v("CTF manifest exceeds maxEntries");
	let n = Sr(e.seed, e.operationId, "manifest"), r = t.map((e, t) => dr(n, e, t)), i = r.map(({ entry: e }) => e);
	return {
		entries: r,
		serialized: i,
		commitment: Yt(i)
...
function ar(e, t) {
	let n = Sr(e, t, "refund"), r = l.Point.CURVE().n;
	for (let e = 0; e < 65536; e += 1) {
		let t = u(a, n, Cr(e)), i = D.toBigInt(t);
		if (!(i === 0n || i >= r)) return {
			privateKey: Va(i),
...
function or(e) {
	...
	hr(e.expiry, e.expiryContext);
	let t = Sr(e.seed, e.operationId, "authorization");
...
function lr(e) {
	...
	return _r(e.outputRecovery) ? "confirmed" : e.outputRecovery.restoredOutputBs.length > 0 || !yr(e.inputStates, e.expectedInputYs) || !e.inputStates.every(({ state: e }) => e === "UNSPENT") ? "reconciling" : e.now >= e.expiry ? "refundable" : "waiting";
}
function ur(e, t, n, r) {
...
function hr(e, t) {
	let n = xr(e, "expiry"), r = xr(t.now, "now"), i = xr(t.maxExpirySeconds, "maxExpirySeconds");
	...
	let o = r + i, s = a.flatMap(({ final_expiry: e }) => e === void 0 ? [] : [xr(e, "final_expiry")]).sort((e, t) => e < t ? -1 : +(e > t));
	if (n >= (s[0] === void 0 || o < s[0] ? o : s[0])) throw new v("CTF authorization expiry must precede the effective keyset expiry ceiling");
}
function gr(e) {
	let t = br(e.condition.condition_id, "condition_id"), n = new Set([...Object.values(e.condition.keysets), ...(e.condition.partitions ?? []).flatMap((e) => Object.values(e.keysets ?? {}))]);
...
function _r(e) {
	...
	if (e.query.mode === "known") {
		...
		return n.size > 0 && n.size === e.query.expectedOutputBs.length && vr(e.query.outputs, n) && n.size === t.size && [...t].every((e) => n.has(e));
	}
	let n = new Set(e.query.manifestBs);
	return n.size > 0 && n.size === e.query.manifestBs.length && vr(e.query.outputs, n) && [...t].every((e) => n.has(e));
}
function vr(e, t) {
...
function Sr(e, t, n) {
	...
	return u(a, e, D.concat(D.fromString(nr), Cr(r.length), r, D.fromString(n)));
}
function Cr(e) {
	let t = new Uint8Array(4);
...
var wr = h("Secp256k1_HashToCurve_Cashu_");
function Tr(t) {
	let n = a(D.concat(wr, t)), r = new Uint32Array(1), i = 2 ** 16;
	for (let t = 0; t < i; t++) {
		let t = new Uint8Array(r.buffer), i = a(D.concat(n, t));
		try {
			return z(e(D.concat(new Uint8Array([2]), i)));
		} catch {
			r[0]++;
		}
	}
...
function zr(e, t) {
	...
	let n = Tr(e.secret), r = l.Point.Fn.fromBytes(t);
...
function Qr(e, t) {
	...
	return {
		blinded: e.map((e, t) => {
			let r = z(e), i = ti(r, n, t), a = r.add(l.Point.BASE.multiply(i));
...
function si(e) {
	let t = V(Array.isArray(e.pubkey) ? e.pubkey : [e.pubkey]), n = V(e.refundKeys ?? []);
...
function hi(e, t = C, n) {
	...
	n = n ?? e.secret;
	let r = B(e.secret), i = Ti(r), a = Ei(r);
	wi({
...
function Ti(e) {
	...
	if (V(r).length !== r.length) throw new v("Duplicate main pubkeys are not allowed");
	return r;
}
function Ei(e) {
	let t = (Jr(e, "refund") ?? []).map((e) => oi(e));
	if (V(t).length !== t.length) throw new v("Duplicate refund pubkeys are not allowed");
	return t;
}
function Di(e) {
	let t = Xr(e, "locktime");
...
var Fi = (e, t, n, r) => {
	...
	let i = Tr(e), a = n.add(r.multiply(t.r)), o = l.Point.BASE.multiply(t.r);
...
function pa(e) {
	...
	for (let e of t) switch (e.tag) {
		...
		case Xi:
			...
			break;
		case Zi:
			e.value.length === 1 && e.value[0] === 0 ? n.unit = "sat" : n.unit = H(e.value);
			break;
		case Qi:
			n.singleUse = _a(e.value) === 1;
			break;
		case $i:
			n.mints || (n.mints = []), n.mints.push(H(e.value));
			break;
		case ea:
			n.description = H(e.value);
			break;
		case ta:
...
function Ca(e) {
	let t = [];
	if (e.id && t.push(U(Yi, wa(e.id))), e.amount !== void 0 && t.push(U(Xi, Ta(e.amount))), e.unit && (e.unit === "sat" ? t.push(U(Zi, new Uint8Array([0]))) : t.push(U(Zi, wa(e.unit)))), e.singleUse !== void 0 && t.push(U(Qi, Ea(+!!e.singleUse))), e.mints && e.mints.length > 0) for (let n of e.mints) t.push(U($i, wa(n)));
	if (e.description && t.push(U(ea, wa(e.description))), e.transports && e.transports.length > 0) for (let n of e.transports) t.push(U(ta, ka(n)));
	if (e.nut10 && e.nut10.length > 0) for (let n of e.nut10) t.push(U(na, Aa(n)));
...
function ka(e) {
	...
	t.push(U(ia, r));
	let a = [...i, ...e.tags || []];
	if (a.length > 0) for (let e of a) t.push(U(aa, ja(e)));
...
function Aa(e) {
	let t = [], n = Oa(e.kind);
	if (t.push(U(ca, Ea(n))), t.push(U(la, wa(e.data))), e.tags && e.tags.length > 0) for (let n of e.tags) t.push(U(ua, ja(n)));
...
function Ha(e) {
	return Array.isArray(e) ? e.some((e) => !G(e.id)) : !G(e.id);
}
function Ua(e, t) {
...
function io(e) {
	...
	return Y(e.map((e) => typeof e == "string" ? A.parse(e) : e));
}
function ao(e) {
...
function co(e, t, r) {
	...
	try {
		...
		return Ii(new TextEncoder().encode(e.secret), t, z(e.C), z(a));
	} catch {
		return !1;
	}
}
function lo(e, t, n) {
	let r = Y(e), i = n?.requireDleq ?? !1, a = i ? "Token contains proofs with invalid or missing DLEQ" : "Token contains a proof with an invalid DLEQ", o = [], s = [];
...
function uo(e) {
	let t = new TextEncoder(), n = Y(e.proofs), r = Ae(qa({
		...e,
		proofs: n
	}));
...
function ho(e) {
	return {
		...e,
		input_fee_ppk: X(e.input_fee_ppk, "keyset.input_fee_ppk", void 0),
		final_expiry: X(e.final_expiry, "keyset.final_expiry", void 0)
	};
}
function go(e) {
	return {
		...e,
		input_fee_ppk: X(e.input_fee_ppk, "keys.input_fee_ppk", void 0),
		final_expiry: X(e.final_expiry, "keys.final_expiry", void 0)
	};
}
//#endregion
...
var _o = class e {
	...
	static normalizeNut22(e, t) {
		...
		let n = 100;
		try {
			n = X(e.bat_max_mint, "nuts.22.bat_max_mint", 100);
		} catch {
	...
	static normalizeNut29(e, t) {
		...
		let n = 100;
		try {
			n = X(e.max_batch_size, "nuts.29.max_batch_size", 100);
		} catch {
	...
	checkNut19() {
		...
		if (e && (e?.cached_endpoints?.length || 0) > 0) {
			let t = X(e.ttl, "nuts.19.ttl", null);
			return {
				supported: !0,
				params: {
		...
		try {
			...
			if (No(n)) {
				...
				if (o < ko && (!t || r < t)) {
					...
					if (r + a > t) throw Q.error(`Network Error: request abandoned after ${o} retries`, {
						e: n,
						retries: o
					}), n;
					return o++, Q.info(`Network Error: attempting retry ${o} in ${a}ms`, {
						e: n,
						retries: o,
						delay: a
			...
			throw Q.error("Request failed and could not be retried", { e: n }), n;
		}
	};
	return c();
}
async function Ro(e) {
	...
	try {
		...
		throw Q.error("Failed to parse HTTP response", { err: e }), new y("bad response", _.status, { cause: e });
	}
}
function zo(e) {
...
var Wo = class {
	...
	async getInfo(e) {
		let t = await (e ?? this._request)({
			endpoint: q(this._mintUrl, "/v1/info"),
			onResponseMeta: this._captureResponseMetadata
		});
		return Z.normalizeInfo(t);
	}
	async getLazyMintInfo(e) {
		if (this._mintInfo) return this._mintInfo;
		let t = await this.getInfo(e);
		return this._mintInfo = new Z(t, this._logger), this._mintInfo;
	}
	setMintInfo(e) {
		this._mintInfo = e instanceof Z ? e : new Z(e, this._logger);
	}
	async swap(e, t) {
		let n = await this.requestWithAuth("POST", "/v1/swap", { requestBody: e }, t);
		if (!K(n) || !Array.isArray(n?.signatures)) throw this._logger.error("Invalid response from mint...", {
			data: n,
			op: "swap"
	...
	async mint(e, t, n) {
		...
		if (!K(r) || !Array.isArray(r?.signatures)) throw this._logger.error("Invalid response from mint...", {
			data: r,
			op: `mint.${e}`
		}), new v("Invalid response from mint");
	...
	async mintBatch(e, t, n) {
		...
		if (!K(i) || !Array.isArray(i?.signatures)) throw this._logger.error("Invalid response from mint...", {
			data: i,
			op: `mintBatch.${e}`
		}), new v("Invalid response from mint");
	...
	async check(e, t) {
		...
		if (!K(n) || !Array.isArray(n?.states)) throw this._logger.error("Invalid response from mint...", {
			data: n,
			op: "check"
		}), new v("Invalid response from mint");
	...
	async getKeys(e, t, n) {
		...
		let i = await (n ?? this._request)({
			endpoint: e ? q(r, "/v1/keys", e) : q(r, "/v1/keys"),
			onResponseMeta: this._captureResponseMetadata
		});
		if (!K(i) || !Array.isArray(i.keysets)) throw this._logger.error("Invalid response from mint...", {
			data: i,
			op: "getKeys"
		}), new v("Invalid response from mint");
		return {
	...
	async getKeySets(e) {
		let t = await (e ?? this._request)({
			endpoint: q(this._mintUrl, "/v1/keysets"),
			onResponseMeta: this._captureResponseMetadata
		});
		if (!K(t) || !Array.isArray(t.keysets)) throw this._logger.error("Invalid response from mint...", {
			data: t,
			op: "getKeySets"
		}), new v("Invalid response from mint");
		return {
	...
	async restore(e, t) {
		let n = await (t ?? this._request)({
			endpoint: q(this._mintUrl, "/v1/restore"),
			method: "POST",
			requestBody: e,
			onResponseMeta: this._captureResponseMetadata
		});
		if (!K(n) || !Array.isArray(n?.outputs) || !Array.isArray(n?.signatures)) throw this._logger.error("Invalid response from mint...", {
			data: n,
			op: "restore"
		}), new v("Invalid response from mint");
	...
	async getConditionalKeysets(e = {}, t) {
		...
		if (!K(a) || !Array.isArray(a.keysets)) throw this._logger.error("Invalid response from mint...", {
			data: a,
			op: "getConditionalKeysets"
		}), new v("Invalid response from mint");
		return { keysets: a.keysets.map((e) => ({
			...e,
			input_fee_ppk: X(e.input_fee_ppk, "conditional_keyset.input_fee_ppk", void 0),
			final_expiry: X(e.final_expiry, "conditional_keyset.final_expiry", void 0),
			registered_at: X(e.registered_at, "conditional_keyset.registered_at", void 0)
		})) };
	}
	async getConditions(e = {}, t) {
		...
		if (!K(a) || !Array.isArray(a.conditions)) throw this._logger.error("Invalid response from mint...", {
			data: a,
			op: "getConditions"
		}), new v("Invalid response from mint");
		return a;
	...
	async registerCondition(e, t) {
		...
		if (!K(n) || typeof n.condition_id != "string") throw this._logger.error("Invalid response from mint...", {
			data: n,
	...
	async getCtfCondition(e, t) {
		...
		let n = e.toLowerCase(), r = await this.getCtfConditionResponse(n, t), i = r, a = this.normalizeCtfCondition(K(i.condition) ? i.condition : r);
		if (!K(a) || a.condition_id?.toLowerCase() !== n || !K(a.keysets)) throw this._logger.error("Invalid response from mint...", {
			data: r,
			op: "getCtfCondition"
	...
	normalizeCtfCondition(e) {
		if (K(e.keysets) || !Array.isArray(e.partitions)) return e;
		let t = {};
		for (let n of e.partitions) if (!(!K(n) || !K(n.keysets))) for (let [e, r] of Object.entries(n.keysets)) typeof r == "string" && (t[e] = r);
		return {
			...e,
			keysets: t
		};
	...
	async getCtfConditionResponse(e, t) {
		try {
			...
			let r = await this.requestWithAuth("GET", "/v1/conditions", {}, t), i = (Array.isArray(r) ? r : K(r) && Array.isArray(r.conditions) ? r.conditions : []).find((t) => t.condition_id?.toLowerCase() === e);
			if (!i) throw n;
			return i;
		}
	}
	...
	async ctfConvert(e, t) {
		...
		if (!K(r) || !K(r.signatures)) throw this._logger.error("Invalid response from mint...", {
			data: r,
			op: "ctfConvert"
		}), new v("Invalid response from mint");
	...
	normalizeMintQuoteBolt11Fields(e) {
		e.amount = k.from(e.amount), e.expiry = X(e.expiry, "mintQuoteBolt11.expiry", null);
	}
	normalizeMintQuoteBolt12Fields(e) {
		to(e, "amount"), e.amount = e.amount === null ? null : k.from(e.amount), e.expiry = X(e.expiry, "mintQuoteBolt12.expiry", null), e.amount_paid = k.from(e.amount_paid), e.amount_issued = k.from(e.amount_issued);
	}
	normalizeMintQuoteOnchainFields(e) {
		e.expiry = X(e.expiry, "mintQuoteOnchain.expiry", null), e.amount_paid = k.from(e.amount_paid), e.amount_issued = k.from(e.amount_issued);
	}
	...
	normalizeMeltBaseFields(e, t) {
		if (e.amount = k.from(e.amount), e.expiry = X(e.expiry, "meltQuote.expiry", void 0), e.change && (e.change = this.normalizeSignatureAmounts(e.change)), !K(e) || typeof e.quote != "string" || !(e.amount instanceof k) || typeof e.unit != "string" || typeof e.state != "string" || typeof e.expiry != "number" || !Object.values(yo).includes(e.state)) throw this._logger.error("Invalid response from mint...", {
			data: e,
			op: t
		}), new v("Invalid response from mint");
	}
	...
	static verifyKeysetId(e) {
		try {
			...
			let t = Ce(e.id) && !G(e.id), r = G(e.id) ? n(e.id)[0] : 0;
			return Qa(e.keys, {
...
var Zo = class e {
	...
	addLockPubkey(e) {
		...
		return this.lockKeys = V([...this.lockKeys, ...t]), this;
	}
	addRefundPubkey(e) {
		let t = Array.isArray(e) ? e : [e];
		return this.refundKeys = V([...this.refundKeys, ...t]), this;
	}
	lockUntil(e) {
		return this.locktime = Xo(e), this;
	}
...
function Qo(e, t, n, r = !1, i = !1, a = C) {
	let o = Y(e), s = k.from(t), c = s.toNumber(), l = re(), u = null, d = Infinity, f = 0, p = 0, m = (e) => {
		try {
...
var $o = class e {
	...
	createDeterministicData(t, n, r, i, a) {
		return this.createSingleDeterministicData === e.prototype.createSingleDeterministicData ? F.createDeterministicData(t, n, r, i, a) : W(t, i.keys, a).map((e, t) => this.createSingleDeterministicData(e, n, r + t, i.id));
	}
...
function ts(e, t, n, r) {
	...
	let c = i.subtract(o);
	if (!c.isZero()) for (let e of W(c, n)) a.push(e), o = o.add(e);
...
function ss(e) {
	return e instanceof Error ? e : new v(typeof e == "string" ? e : os(e), { cause: e });
}
function cs() {
...
var ls = class {
	...
	waitUntilPaid(e, t, n, r = "Timeout waiting for paid") {
		return new Promise((i, a) => {
			let o = null, s = null, c = !1, l = (e) => {
				c || (c = !0, $(o), s && (clearTimeout(s), s = null), n?.signal && n.signal.removeEventListener("abort", u), e && a(ss(e)));
			}, u = () => l(cs());
			if (n?.signal) {
	...
	async proofStateUpdates(e, t, n, r) {
		...
		for (let t of e) {
			let e = P(t.id) ? N(a.encode(t.secret)).toHex(!0) : Tr(a.encode(t.secret)).toHex(!0);
		...
		return new Promise((n, r) => {
			let i = Array.from(new Set(e)), a = /* @__PURE__ */ new Map(), o = null, s = null, c = !1, l = !1, u = (e) => {
				if (!l) {
					...
					a.clear(), o && (clearTimeout(o), o = null), t?.signal && t.signal.removeEventListener("abort", d), e && r(ss(e));
				}
			}, d = () => u(cs());
			if (t?.signal) {
	...
	send(e, t) {
		return new ds(this.wallet, e, t);
	}
	receive(e) {
		return new fs(this.wallet, e);
	}
	mintBolt11(e, t) {
		return new ps(this.wallet, "bolt11", e, t);
	}
	mintBolt12(e, t) {
		return new ps(this.wallet, "bolt12", e, t);
	}
	mintOnchain(e, t) {
		return new ps(this.wallet, "onchain", e, t);
	}
	meltBolt11(e, t) {
		return new ms(this.wallet, "bolt11", e, t);
	}
	meltBolt12(e, t) {
		return new ms(this.wallet, "bolt12", e, t);
	}
	meltOnchain(e, t) {
		return new hs(this.wallet, e, t);
	}
}, ds = class {
	constructor(e, t, n) {
...
}, gs = "__PENDING__", _s = class e {
	constructor(e, t) {
		this._seed = void 0, this._unit = "sat", this._mintInfo = void 0, this._denominationTarget = 3, this._secretsPolicy = "auto", this._boundKeysetId = gs, this._requireSigDleq = !1, this.ops = new us(this), this.on = new ls(this), this._logger = t?.logger ?? C, this._selectProofs = t?.selectProofs ?? Qo, this._outputDataCreator = t?.outputDataCreator ?? new $o(), this.mint = typeof e == "string" ? new qo(e, {
			authProvider: t?.authProvider,
			logger: this._logger
		}) : e, this._unit = t?.unit ?? this._unit, this._boundKeysetId = t?.keysetId ?? this._boundKeysetId, t?.bip39seed && (this.failIf(!(t.bip39seed instanceof Uint8Array), "bip39seed must be a valid Uint8Array", { bip39seed: t.bip39seed }), this._seed = t.bip39seed), this._secretsPolicy = t?.secretsPolicy ?? this._secretsPolicy, t?.counterSource ? this._counterSource = t.counterSource : this._counterSource = new rs(t?.counterInit), this.counters = new as(this._counterSource), this._keyChain = new Yo(this.mint, this._unit), this._denominationTarget = t?.denominationTarget ?? this._denominationTarget, this._requireSigDleq = t?.requireSigDleq ?? this._requireSigDleq, this.ctf = t?.enableCtf ? this.createCtfFacade() : void 0;
	}
	createCtfFacade() {
		return {
	...
	async loadMint(e) {
		let t = [];
		(!this._mintInfo || e) && t.push(this.mint.getInfo().then((e) => (this._mintInfo = new Z(e, this._logger), this.mint.setMintInfo(this._mintInfo), null))), t.push(this._keyChain.init(e)), await Promise.all(t), this.finishInit();
	}
	loadMintFromCache(e, t) {
		this._mintInfo = new Z(e, this._logger), this.mint.setMintInfo(this._mintInfo), this._keyChain.loadFromCache(t), this.finishInit();
	}
	finishInit() {
		if (this._logger.debug("KeyChain", { keychain: this._keyChain.cache }), this._boundKeysetId === gs) try {
	...
	get keysetId() {
		return this.failIf(this._boundKeysetId === gs, "Wallet has no bound keyset. The mint may have no active keysets, or wallet was not initialized via loadMint or loadMintFromCache"), this._boundKeysetId;
	}
	getKeyset(e) {
	...
	configureOutputs(e, t, n, r = !1, i = []) {
		...
		if (s.length === 0 && o.length > 0 && (s = ts(o, a, t.keys, this._denominationTarget)), s = W(a, t.keys, s), r) {
			let e = this.getFeesForKeyset(s.length, t.id), n = W(e, t.keys);
			for (; this.getFeesForKeyset(s.length + n.length, t.id).greaterThan(e);) e = e.add(1), n = W(e, t.keys);
			a = a.add(e), s = [...s, ...n];
		}
		return {
			...n,
	...
	createOutputData(e, t, n) {
		...
		switch (n.type) {
			...
			case "p2pk":
				...
				break;
			case "factory":
				i = W(r, t.keys, n.denominations).map((e) => n.factory(e, t));
				break;
			case "custom": {
				i = n.data;
	...
	async prepareSwapToReceive(e, t, n) {
		...
		let u = this.parseAmount(J(s), "prepareSwapToReceive", !0);
	...
	sendOffline(e, t, n) {
		let r = this.parseAmount(e, "sendOffline"), i = Y(t), { requireDleq: a = !1, includeFees: o = !1, exactMatch: s = !0 } = n || {};
		a && (i = i.filter((e) => P(e.id) || e.dleq != null)), this.failIf(J(i).lessThan(r), "Not enough funds available to send");
	...
	async send(e, t, n, r) {
		...
		try {
			...
			if (J(c).equals(i.add(l))) return this._logger.info("Successful exactMatch offline selection!"), {
				keep: s,
				send: c
			};
		} catch (e) {
	...
	async prepareSwapToSend(e, t, n, r) {
		let i = this.parseAmount(e, "prepareSwapToSend"), a = Y(t), { keysetId: o, includeFees: s = !1, onCountersReserved: c } = n || {};
		r = r ?? {
			send: this.defaultOutputType(),
			keep: this.defaultOutputType()
		};
		...
		let m = J(p), h = this.getFeesForProofs(p), g = d.add(h);
	...
	async prepareConditionalSwap(e) {
		...
		}), c = k.from(Math.ceil(t.length * i.fee / 1e3)), l = J(t).subtract(c);
	...
	maxSpendableAfterFees(e, t = 0) {
		let n = J(e), r = this.getFeesForProofs(e).add(t);
	...
	async meltProofsOnchain(e, t, n, r) {
		...
		let a = Y(t), o = this.getFeesForProofs(a), s = J(a), c = e.amount.add(i.fee_reserve).add(o);
	...
	async prepareMelt(e, t, n, r, i) {
		...
		let { keysetId: a, onCountersReserved: o, nut08Change: s = !0 } = r || {}, c = this.getKeyset(a), l = Y(n), u = J(l), d = [];
	...
	async checkProofsStates(e) {
		let t = new TextEncoder(), n = e.map((e) => P(e.id) ? N(t.encode(e.secret)).toHex(!0) : Tr(t.encode(e.secret)).toHex(!0)), r = [];
	...
	async init() {
		if (!this.info) {
			let e = await this.req({
				endpoint: q(this.mintUrl, "/v1/info"),
				method: "GET"
			});
			this.info = new Z(e, this.logger);
		}
		if (!this.keychain) {
			let [e, t] = await Promise.all([this.req({
				endpoint: q(this.mintUrl, "/v1/auth/blind/keysets"),
				method: "GET"
			}), this.req({
				endpoint: q(this.mintUrl, "/v1/auth/blind/keys"),
				method: "GET"
			})]), n = e.keysets.map((e) => ho(e)), r = t.keysets.map((e) => go(e)), i = Yo.mintToCacheDTO(this.mintUrl, n, r);
			this.keychain = Yo.fromCache(this.mintUrl, "auth", i), this.keychain.getCheapestKeyset();
		}
	}
	getBatMaxMint() {
		if (!this.info) throw new v("AuthManager: mint info not loaded");
		let e = this.info.nuts[22], t = X(e?.bat_max_mint, "nuts.22.bat_max_mint", this.maxPerMint);
	...
	async topUp(e) {
		...
		let s = await this.req({
			endpoint: q(this.mintUrl, "/v1/auth/blind/mint"),
			method: "POST",
			headers: o,
			requestBody: a
		});
...
function ws(e) {
	...
	let n = A.stringify(t) ?? "{}";
	return `${Ss}${ve(D.fromString(n))}`;
}
function Ts(e, t) {
	if (!e.startsWith(Ss)) throw new v(`Invalid signing package: must start with "${Ss}"`);
	let n = e.slice(7), r;
	try {
	...
	let a = i, o = a.version;
	if (o !== Ss) throw new v(`Invalid signing package version: ${o}`);
	let s = a.type;
	...
	if (t?.validateDigest) {
		let e = Cs(a.inputs, a.outputs, a.quote);
...
function Ds(e) {
	...
	return ks("swap", e.inputs, t.map((e) => e.blindedMessage));
}
function Os(e) {
	return ks("melt", e.inputs, e.outputData.map((e) => e.blindedMessage), e.quote.quote);
}
function ks(e, t, n, r) {
	let i = Cs(t, n, r), a = $e(yi(t, n.map((e) => ({ blindedMessage: e })), r), !0);
...
var Ns = {
	computeDigests: Cs,
	extractSwapPackage: Ds,
	extractMeltPackage: Os,
	serializePackage: ws,
	deserializePackage: Ts,
	signPackage: Es,
	signDigest: M,
	mergeSwapPackage: As,
	mergeMeltPackage: js
...
//#endregion
export { k as Amount, O as AmountError, oe as AmountWithUnit, ae as AmountWithUnitError, ys as AuthManager, ct as BLS_FR_ORDER, lt as BLS_G2_GENERATOR, st as BLS_HASH_TO_CURVE_DST, v as CTSError, qo as CashuMint, qo as Mint, _s as CashuWallet, _s as Wallet, bo as CheckStateEnum, ne as ConsoleLogger, y as HttpResponseError, A as JSONInt, Yo as KeyChain, Jo as Keyset, ms as MeltBuilder, hs as MeltOnchainBuilder, yo as MeltQuoteState, ps as MintBuilder, Z as MintInfo, S as MintOperationError, vo as MintQuoteState, b as NetworkError, _o as OIDCAuth, F as OutputData, Zr as P2BK_DST, Zo as P2PKBuilder, Fa as PaymentRequest, Ji as PaymentRequestTransportType, x as RateLimitError, fs as ReceiveBuilder, ds as SendBuilder, Ns as SigAll, ni as SigFlags, Ko as WSConnection, as as WalletCounters, ls as WalletEvents, us as WalletOps, Et as asBlsG1Point, Tt as asSecpPoint, kt as assertCanonicalKeysetId, Hr as assertSecretKind, vi as assertSigAllInputs, wt as batchVerifyUnblindedSignatureBls, Ua as bigIntStringify, Mr as blindMessage, vt as blindMessageBls, sr as buildCtfRangeRecoveryQuery, ji as buildLegacyP2PKSigAllMessage, yi as buildP2PKSigAllMessage, lr as classifyCtfSettlementRecovery, Yt as computeCtfManifestCommitment, Jt as computeCtfReceiveCommitment, tn as computeCtfSettlementCoordinatorDigest, en as computeCtfSettlementRequestDigest, $e as computeMessageDigest, ln as computePayToUnlockRefundDigest, Pr as constructUnblindedSignature, bt as constructUnblindedSignatureBls, xs as createAuthWallet, Ar as createBlindSignature, xt as createBlindSignatureBls, or as createCtfAuthorizationOutputs, on as createCtfPayToUnlockSecret, ir as createCtfRangeManifest, Xt as createCtfSelectionBitmap, Li as createDLEQProof, is as createEphemeralCounterSource, Bi as createHTLCHash, Ri as createHTLCsecret, Rr as createNewMintKeys, ai as createP2PKsecret, jr as createRandomRawBlindedMessage, kr as createRandomSecretKey, Br as createSecret, Zn as createSecretAndBlindingFactorDeriver, ao as decodePaymentRequest, V as dedupeP2PKPubkeys, Ct as deriveBatchWeights, Yn as deriveBlindingFactor, at as deriveConditionalKeysetId, cr as deriveCtfRangeRecoverySelection, ar as deriveCtfRangeRefundKey, Qa as deriveKeysetId, Qr as deriveP2BKBlindedPubkeys, ei as deriveP2BKSecretKey, $r as deriveP2BKSecretKeys, Jn as deriveSecret, Xn as deriveSecretAndBlindingFactor, Lr as deserializeMintKeys, io as deserializeProofs, nt as findSigningKey, Gr as getDataField, Ya as getDecodedToken, fo as getDecodedTokenBinary, Ga as getEncodedToken, Ga as getEncodedTokenV4, uo as getEncodedTokenBinary, gt as getG2PubKeyFromPrivKey, Wi as getHTLCWitnessPreimage, Ia as getKeysetAmounts, At as getKeysetIdInt, ci as getP2PKExpectedWitnessPubkeys, li as getP2PKSigFlag, ui as getP2PKWitnessSignatures, Or as getPubKeyFromPrivKey, Wr as getSecretData, Ur as getSecretKind, Jr as getTag, Xr as getTagInt, Yr as getTagScalar, Kr as getTags, Xa as getTokenMetadata, rt as getValidSigners, Ra as hasCorrespondingKey, mi as hasP2PKSignedProof, qr as hasTag, co as hasValidDleq, Tr as hashToCurve, N as hashToCurveBls, Er as hash_e, Ba as hexToNumber, Ho as injectWebSocketImpl, mo as invoiceHasAmountInHRP, P as isBlsKeyset, Ui as isHTLCSpendAuthorised, K as isObj, bi as isP2PKSigAll, gi as isP2PKSpendAuthorised, G as isValidHex, q as joinUrls, _i as maybeDeriveP2BKPrivateKeys, it as meetsSignerThreshold, si as normalizeP2PKOptions, Y as normalizeProofAmounts, no as normalizeUrl, to as nullIfUndefined, Va as numberToHexPadded64, sn as parseCtfPayToUnlockCondition, Zt as parseCtfSelectionBitmap, zi as parseHTLCSecret, B as parseP2PKSecret, Vr as parseSecret, di as parseWitnessData, Dr as pointFromBytes, z as pointFromHex, Dt as pointFromHexAuto, mt as pointFromHexG1, ht as pointFromHexG2, Ot as pointToHex, ur as recoverCtfRangeProofs, M as schnorrSignDigest, et as schnorrSignMessage, tt as schnorrVerifyMessage, Qt as selectCtfManifestOutputs, $t as selectCtfRangeAmounts, Qo as selectProofsRGLI, Ir as serializeMintKeys, ro as serializeProofs, Do as setGlobalRequestOptions, nn as signCtfSettlementRequest, Ki as signMintQuote, pi as signP2PKProof, fi as signP2PKProofs, un as signPayToUnlockRefund, eo as sortProofsById, W as splitAmount, oo as stripDleq, J as sumProofs, Nr as unblindSignature, yt as unblindSignatureBls, cn as validateCtfPoolPolicyTotals, rn as verifyCtfSettlementCoordinatorSignature, Fi as verifyDLEQProof, Ii as verifyDLEQProof_reblind, Vi as verifyHTLCHash, Hi as verifyHTLCSpendingConditions, qi as verifyMintQuoteSignature, hi as verifyP2PKSpendingConditions, lo as verifyProofsForReceive, zr as verifyUnblindedSignature, St as verifyUnblindedSignatureBls };
